...
void ApplicationWindow::onImport()
{
	...
	if (stat)
	{
		DocumentCommon* doc = qobject_cast<OccDocQt*>(getWorkspace()->activeSubWindow()->widget())->getDocument();
		doc->fitAll();
	}
}
...
{
	OccDocQt* w = qobject_cast<OccDocQt*>(getWorkspace()->activeSubWindow()->widget());
	if (w)
		w->dump();
}