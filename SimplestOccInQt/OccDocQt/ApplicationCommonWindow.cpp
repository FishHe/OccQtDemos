...
{
	OccDocQt* aWindow = qobject_cast<OccDocQt*> (stWs->activeSubWindow()->widget());
...
{
	OccDocQt* window = qobject_cast<OccDocQt*>(stWs->activeSubWindow()->widget());
...
void ApplicationCommonWindow::onWindowActivated(QWidget * w)
{
	...
	OccDocQt* window = qobject_cast<OccDocQt*>(w);
...
void ApplicationCommonWindow::onToolAction()
{
	...
	DocumentCommon* doc = qobject_cast<OccDocQt*>(ws->activeSubWindow()->widget())->getDocument();
...
void ApplicationCommonWindow::onSelectionChanged()
{
	...
	if (!qobject_cast<OccDocQt*>(ws->activeSubWindow()->widget()))
		return;

	doc = (qobject_cast<OccDocQt*>(ws->activeSubWindow()->widget()))->getDocument();
